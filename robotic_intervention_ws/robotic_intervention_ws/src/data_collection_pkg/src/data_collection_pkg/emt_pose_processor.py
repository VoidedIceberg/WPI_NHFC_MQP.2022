...
class EmtDataProcessor:
    ...
    def get_tf_between(self, reference_frame, target_frame):
        try:
            transformation = self.tfBuffer.lookup_transform(reference_frame, target_frame, rospy.Time(0))
...
class EmtDataFormatter:
    ...
    def convert_tf_to_2D(self, relative_frame):
        ...
        translation_mm = (x1 * 1000)
        rotation_deg = ((absolute_roll * 180) / (np.pi))
